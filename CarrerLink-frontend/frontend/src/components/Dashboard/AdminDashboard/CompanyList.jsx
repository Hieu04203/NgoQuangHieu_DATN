...
const CompanyList = () => {
    ...
    useEffect(() => {
        const fetchCompanies = async () => {
            try {
                ...
                Swal.fire('Error!', 'Không tải được công ty', 'error');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleApprove = async (companyId) => {
        const result = await Swal.fire({
            ...
            text: 'Bạn có chắc chắn muốn chấp thuận công ty này không?',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Vâng, chấp thuận!'
        });
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Danh sách công ty</h2>
                    ...
                    <div key={company.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-xl hover:border-indigo-500 transition-colors">
                        <div className="space-y-1">
                            <div className="flex items-center space-x-4">
                                ...
                                <span className={`px-2 py-1 text-sm rounded-full ${company.status ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                    {company.status ? 'Đã phê duyệt' : 'Chưa phê duyệt'}
                                </span>
                            </div>
