...
const JobFieldManagement = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (response.success) {
            await loadJobFields();
            setFieldName('');
            Swal.fire('Thành công!', 'Đã thêm trường công việc thành công', 'Thành công');
        } else {
            Swal.fire('Lỗi!', response.message, 'Lỗi');
        }
    };

    return (
        <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Quản lý các trường công việc</h2>

            <form onSubmit={handleSubmit} className="flex gap-4 mb-6">
                <input
                    type="text"
                    placeholder="Nhập tên trường công việc"
                ...
                >
                    <Plus size={20} className="mr-2" /> Thêm trường công việc
                </button>
            </form>
