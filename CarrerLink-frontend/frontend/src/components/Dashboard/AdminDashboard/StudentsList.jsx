...
const StudentList = () => {
    ...
    const handleApprove = async (studentId) => {
        const result = await Swal.fire({
            ...
            text: 'Bạn có chắc chắn muốn chấp thuận ứng viên này không?',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Có, chấp thuận!'
        });
                        ...
                        <div className="space-y-1">
                            <div className="flex items-center space-x-4">
                                ...
                                <span className={`px-2 py-1 text-sm rounded-full ${student.approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                    {student.approved ? 'Đã phê duyệt' : 'Chưa phê duyệt'}
                                </span>
                            </div>
                            <p className="text-gray-600">{student.email}</p>
                            <p className="text-gray-600 text-sm">Ngành: {student.degree}</p>
                        </div>
                        <Button
