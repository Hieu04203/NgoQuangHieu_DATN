  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
      <div className="flex justify-between mb-4">
        <h2 className="text-2xl font-semibold text-[#1a4d5c]">Câu hỏi TEST</h2>
        <div className="text-lg font-medium">
        Thời gian còn lại: <span className="text-red-600">{formatTime(timeLeft)}</span>
        </div>
      </div>
      <div className="mb-4">
        <p className="font-medium">Câu hỏi {currentQuestionIndex + 1} of {totalQuestions}</p>
        <p className="text-sm">Thời gian còn lại cho câu hỏi này: {formatTime(questionTimeLeft)}</p>
      </div>
      <div className="mb-6">
